...
class Chat(Base):
    ...
    caller: Mapped[Caller] = relationship(back_populates="chats", lazy="noload")
...
class ChatInputModel(BaseModel):
    ...
    caller_session_id: (
        Annotated[str, StringConstraints(max_length=Chat.caller_session_id.type.length)]
        | None
    ) = Field("<chat session id>")
    caller_chat_text: str = Field("")
    caller_attachment_type: AttachmentType | None = Field(None)
