...
class CLIArgs:
    ...
    inference_provider_type: InferenceProviderType = InferenceProviderType.IN_PROCESS
...
def parse_cli_args_with_defaults() -> CLIArgs:
    ...
    parser.add_argument(
        ...
        help="Inference provider type: 'in_process' (default)",
    )
    parser.add_argument(
        "--messaging-provider-type",
        help="Messaging provider type: 'whatsapp_for_business' (default)",
    )
    ...
    cli_args = {
        ...
        "messaging_provider_type": (
            parse_strenum_from_string(
                MessagingProviderType, args.messaging_provider_type
            )
            if args.messaging_provider_type
            else None
        ),
    }
    result = CLIArgs(
...
def is_callable(obj: object, method: str):
    """Check if method is callable in object"""
    return hasattr(obj, method) and callable(getattr(obj, method))


def extend_method(cls):
    """Extend method to class"""

    def decorator(func: callable) -> callable:
        setattr(cls, func.__name__, func) if not hasattr(cls, func.__name__) else None
        return func
